...
export const LoginForm = () => {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (user) {
        // Store user data in localStorage (in a real app, you'd use a proper auth state management)
        localStorage.setItem('socialBandUser', JSON.stringify({ email: user.email, name: user.name }));
        toast({
  ...
  return (
    <Card className="w-full max-w-md">
      <CardHeader>
        <CardTitle className="text-2xl font-display">Login to SocialBand</CardTitle>
